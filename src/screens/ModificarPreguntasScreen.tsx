...
export default function ModificarPreguntasScreen({ onBack }: Props) {
  const [sets, setSets] = useState<SetPreguntas[]>([]);
  const [selectedSetIdx, setSelectedSetIdx] = useState(0);
  const [preguntas, setPreguntas] = useState<Pregunta[]>([]);
  const [preguntasEdicion, setPreguntasEdicion] = useState<Record<string, string>>({});
  ...
  const loadPreguntas = useCallback(async () => {
    ...
    if (error) {
      ...
      ps.forEach(p => { map[p.id] = p.texto ?? ''; });
      setPreguntasEdicion(map);
    }
  ...
  const guardar = async () => {
    ...
    try {
      const updates = preguntas.map(p =>
        supabase
          .from('pregunta')
          .update({ texto: preguntasEdicion[p.id]?.trim() || null })
          .eq('id', p.id)
      );
      ...
      Alert.alert('¡Listo!', 'Preguntas guardadas correctamente.');
    } catch (err: any) {
    ...
    <ScreenLayout>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Modificar preguntas</Text>
        </View>
                ...
                <View style={styles.preguntaHeaderRow}>
                  ...
                  <View style={styles.inputWrapper}>
                    ...
                    <TextInput
                      ...
                      value={preguntasEdicion[p.id] ?? ''}
                      onChangeText={text =>
                        setPreguntasEdicion(prev => ({ ...prev, [p.id]: text }))
                      }
